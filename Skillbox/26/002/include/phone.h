...
class Cellphone {
  enum CMD { ADD, CALL, SMS, EXIT, NONE };
  ...
  Contact *findNumber() {
    ...
    for (int i = 0; i < contacts.size(); ++i)
      if (useNumber && contacts[i].getNumber() == name)
        return &contacts[i];
      else if (!useNumber && contacts[i].getName() == name)
        return &contacts[i];
  ...
  void call() {
    ...
    std::cout << "CALL +7" << contact->getNumber() << std::endl;
  }
  void sms() {
    std::string smsText;
    std::cout << "Input name or number:";
...
#define CMDSTRTOINT(comm) (input == CMD_STR[comm]) command = comm;
    if (input == CMD_STR[ADD])
      command = ADD;
    else if CMDSTRTOINT (CALL)
    else if CMDSTRTOINT (SMS) else if CMDSTRTOINT (EXIT) else if CMDSTRTOINT (
          NONE)
#undef CMDSTRTOINT
              if (command == NONE) std::cout
          << "Try again" << std::endl;
    return command;
  }
